...
import { MIDDLEGAME_LESSONS, type MiddlegameLesson } from "@/lib/middlegame";
import { explainMiddlegame } from "@/lib/coach.functions";
import { supabase } from "@/integrations/supabase/client";
import { speak, stopSpeaking, type Lang } from "@/lib/voice";
import { Brain, GraduationCap, Volume2, VolumeX, ChevronLeft, ChevronRight, Send, Mic, MicOff } from "lucide-react";
import { listen, type RecognitionHandle } from "@/lib/voice";
import { toast } from "sonner";

export const Route = createFileRoute("/_authenticated/middlegame")({
  component: MiddlegamePage,
});

const LEVELS = ["Beginner", "Intermediate", "Advanced"] as const;

function MiddlegamePage() {
  const [selected, setSelected] = useState<MiddlegameLesson | null>(null);
  return (
    <AppShell title="Middlegame Academy">
      {!selected ? <LessonPicker onPick={setSelected} /> : <LessonView lesson={selected} onExit={() => setSelected(null)} />}
    </AppShell>
  );
}

function LessonPicker({ onPick }: { onPick: (l: MiddlegameLesson) => void }) {
  const [level, setLevel] = useState<(typeof LEVELS)[number]>("Beginner");
  const filtered = useMemo(() => MIDDLEGAME_LESSONS.filter((l) => l.level === level), [level]);
  return (
    <div>
      <p className="text-muted-foreground mb-4 max-w-2xl">
        Position study with Guru. Aap pehle <strong>khud socho</strong> — plan, weak squares, piece activity — phir Guru aapki thinking pe feedback dega. Voice on hai by default.
      </p>
        ...
        {filtered.map((l) => (
          ...
          >
            <div className="flex items-center justify-between mb-2">
              <span className="font-display text-lg gold-gradient">{l.title}</span>
              <Brain className="h-4 w-4 text-muted-foreground group-hover:text-primary" />
            </div>
            <div className="flex items-center gap-2 text-xs text-muted-foreground mb-3 flex-wrap">
              <span className="px-2 py-0.5 rounded bg-secondary">{l.theme}</span>
              <span className="px-2 py-0.5 rounded bg-secondary">{l.side === "white" ? "♔ White" : "♚ Black"}</span>
            </div>
            <p className="text-sm text-muted-foreground line-clamp-2">{l.context}</p>
          </button>
        ))}
      </div>
    </div>
...
function LessonView({ lesson, onExit }: { lesson: MiddlegameLesson; onExit: () => void }) {
  const [qIdx, setQIdx] = useState(0);
  const [studentText, setStudentText] = useState("");
  const [coachMsg, setCoachMsg] = useState<string>(
    `**${lesson.title}**\n\n${lesson.context}\n\nPehle socho — ${lesson.questions[0]}\n\nApna jawab niche likho ya mic dabao. Voice on hai.`,
  );
  const [coachLoading, setCoachLoading] = useState(false);
  const [lang, setLang] = useState<Lang>("hinglish");
  const [voiceOut, setVoiceOut] = useState(true);
  const explain = useServerFn(explainMiddlegame);
  ...
  const submitThought = async () => {
    const thought = studentText.trim() || "(no answer yet — please teach the concept from scratch using the Socratic method)";
    ...
    try {
      const res = await explain({
        data: {
          ...
          studentThought: `Question asked: "${lesson.questions[qIdx]}"\nMy thinking: ${thought}`,
          lang,
        },
      });
  ...
  const nextQuestion = () => {
    ...
    if (next >= lesson.questions.length) {
      setCoachMsg(`✅ **Lesson complete!**\n\nKey takeaways:\n${lesson.keyIdeas.map((k) => `- ${k}`).join("\n")}\n\nIs concept ko apni next game mein apply karne ki koshish karo.`);
      return;
    }
    setQIdx(next);
    setCoachMsg(`Next question — ${lesson.questions[next]}\n\nSocho, phir jawab do.`);
  };
    ...
    <div>
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between text-xs text-muted-foreground mb-1">
          <span>{lesson.title} — {lesson.theme} ({lesson.level})</span>
          <span>Q {Math.min(qIdx + 1, lesson.questions.length)} / {lesson.questions.length}</span>
        </div>
        <div className="h-1.5 bg-secondary rounded-full overflow-hidden">
          <div className="h-full bg-primary transition-all" style={{ width: `${Math.round(((qIdx + 1) / lesson.questions.length) * 100)}%` }} />
        </div>
      </div>
        ...
        <div className="space-y-4">
          <div className="card-elevated rounded-xl p-4 border border-primary/30">
            ...
            {!coachLoading && (
              ...
              >
                {qIdx + 1 >= lesson.questions.length ? "Lesson summary" : "Next question"} <ChevronRight className="h-4 w-4" />
              </button>
            )}
          </div>
          ...
          <div className="card-elevated rounded-xl p-4 space-y-2">
            ...
            <textarea
              ...
              rows={4}
              placeholder="Apna plan, candidate moves, weak squares yaha likho…"
