...
import { analyzePgn } from "@/lib/coach.functions";
...
function AnalyzePage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await analyze({ data: { pgn } });
      setResult(r);
    } catch (e: any) {
  ...
  return (
    <AppShell title="Analyze a Game">
      <div className="grid md:grid-cols-2 gap-6">
        <div className="card-elevated rounded-xl p-5 space-y-3">
          <div className="text-sm text-muted-foreground">Paste your PGN below. Guru will review it like a coach — not an engine dump.</div>
          <textarea
            value={pgn}
        ...
        <div className="card-elevated rounded-xl p-5 min-h-72">
